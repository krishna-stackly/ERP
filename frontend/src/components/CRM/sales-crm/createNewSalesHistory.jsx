...
export default function CreateNewSalesHistory({ history = [] }) {
  const statusChanges = history.filter((h) => h.event_type === "status_change");
  const emailPrint    = history.filter((h) => h.event_type === "email" || h.event_type === "print");
  ...
  return (
    <div className="createNewSales-history-container">
      ...
      <div className="createNewSales-statusChange-container">
        <div className="createNewSales-status-head">
          <svg className="createNewSales-status-logo" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none">
        ...
        <div className="createNewSales-status-detailes">
          {statusChanges.length > 0 ? (
            statusChanges.map((h, ind) => (
              <nav key={ind}>
                <ProfileIcon />
                <p>
                  — Status changed to <strong>{h.status}</strong>
                  {h.action_by ? ` by ${h.action_by}` : ""}
      ...
      <div className="createNewSales-statusChange-container">
        <div className="createNewSales-status-head">
          <svg className="createNewSales-history-mail-logo" xmlns="http://www.w3.org/2000/svg" width="20" height="16" viewBox="0 0 20 16" fill="none">
        ...
        <div className="createNewSales-status-detailes">
          {emailPrint.length > 0 ? (
            emailPrint.map((h, ind) => (
              <nav key={ind}>
                <ProfileIcon />
                <p>
                  — {h.event_type === "email" ? "Email sent" : "PDF generated"}
                  {h.action_by ? ` by ${h.action_by}` : ""}
                  {h.timestamp ? ` (${formatTimestamp(h.timestamp)})` : ""}
                </p>
              </nav>
