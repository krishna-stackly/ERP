...
export default function CreateNewDeliverySerialNum({
  setShowSerial,
  itemIndex,
  itemData,
  onConfirm,
}) {
  const availableSerials = itemData?.available_serials || [];
  const alreadySelected  = itemData?.serial_numbers    || [];
  const required         = parseInt(itemData?.quantity) || 0;

  const [checkedItems, setCheckedItems] = useState(() => {
    const init = {};
    alreadySelected.forEach((s, i) => {
      const idx = availableSerials.findIndex(
        (a) => a.serial_no === (typeof s === "object" ? s.serial_no : s)
      );
      if (idx >= 0) init[idx] = true;
    });
    return init;
  });
  ...
  const filtered = availableSerials.filter((s) =>
  ...
  const handleCheckboxChange = (index) => {
    if (checkedItems[index]) {
      setCheckedItems((prev) => ({ ...prev, [index]: false }));
    } else if (totalChecked < required) {
      setCheckedItems((prev) => ({ ...prev, [index]: true }));
    } else {
      toast.warning(`You can only select ${required} serial numbers.`);
    }
  };
  ...
  const handleSelectAll = (e) => {
    ...
    if (e.target.checked) {
      let count = 0;
      for (let i = 0; i < availableSerials.length && count < required; i++) {
        newChecked[i] = true;
        count++;
      }
    } else {
      availableSerials.forEach((_, i) => (newChecked[i] = false));
    }
  ...
  const handleConfirm = () => {
    const selected = availableSerials
      ...
      .map((s) => ({ serial_no: s.serial_no }));
    onConfirm(itemIndex, selected);
    toast.success("Serial numbers confirmed!");
  };
  ...
  return (
    <div className="createNewDelivery-serial-conratner">
              ...
              <th>
                <div>
                  <input
                    type="checkbox"
                    onChange={handleSelectAll}
                    checked={totalChecked === required && required > 0}
          ...
          <tbody className="createNewDelivery-serial-tbody">
            {filtered.length > 0 ? (
              filtered.map((ele, ind) => {
                const originalIndex = availableSerials.indexOf(ele);
                return (
                  <tr key={ind}>
                    <td>{ind + 1}</td>
                    <td>{ele.serial_no}</td>
                    <td>
                      <input
                        type="checkbox"
                        checked={checkedItems[originalIndex] || false}
                        disabled={!checkedItems[originalIndex] && totalChecked >= required}
              ...
              <tr>
                <td colSpan="3">No serial numbers available.</td>
              </tr>
            )}
          </tbody>
        </table>
      ...
      <div className="createNewDelivery-serial-btn-container">
        <p>
          Selected:{" "}
          <span className={totalChecked !== required ? "createNewDelivery-serial-red" : "createNewDelivery-serial-green"}>
            {totalChecked}
          </span>{" "}
          / {required} | Total Available: {availableSerials.length}
        </p>
        <nav>
          <button
            className="createNewDelivery-cancel-btn"
          ...
          <button
            className={totalChecked !== required ? "createNewDelivery-inactive-btn" : "createNewDelivery-active-btn"}
            onClick={handleConfirm}
            disabled={totalChecked !== required}
          >
            Confirm
          </button>
        </nav>
