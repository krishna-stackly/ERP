...
export default function PurchaseAttachment({ poId, inputDisable }) {
  // Files uploaded in this session (File objects, before API save)
  const [uploadedFiles, setUploadedFiles] = useState([]);
  // Files already saved on the backend
  const [backendFiles, setBackendFiles]   = useState([]);
  const [uploading, setUploading]         = useState(false);
  const fileInputRef                      = useRef(null);

  // ── Fetch existing attachments when PO id is available ───────────────────
  useEffect(() => {
    if (!poId) return;
    const loadAttachments = async () => {
      const res = await purchaseOrderApiProvider.fetchAttachments(poId);
      const items = Array.isArray(res)
        ? res
        : res?.results || res?.attachments || [];
      // Normalise to { name, url } shape used in rendering
      setBackendFiles(
        items.map((att) => ({
          name: att.file_name || att.name || att.filename || "Attachment",
          url:  att.file_url  || att.url  || att.file    || "#",
  ...
  // ── File input change: upload immediately via API ─────────────────────────
  const handleFileChange = async (event) => {
    ...
    // Optimistically show in the "uploading" list
    setUploadedFiles((prev) => [...prev, ...newFiles]);
    ...
    for (const file of newFiles) {
      ...
      const result = await purchaseOrderApiProvider.addAttachment(poId, formData);
      if (result) {
        // Move from local list → backend list
        const saved = result?.data || result;
        setBackendFiles((prev) => [
          ...prev,
          {
            name: saved.file_name || saved.name || saved.filename || file.name,
            url:  saved.file_url  || saved.url  || saved.file    || "#",
  ...
  const removeBackendFile = (index) => {
    if (window.confirm("Are you sure you want to remove this file?")) {
      // TODO: call a delete-attachment API endpoint when backend provides one
      setBackendFiles((prev) => prev.filter((_, i) => i !== index));
    }
  };
  ...
  return (
    <div className="createNewPurchase-attachment-container">
      ...
      {/* ── Upload button — identical SVG/markup to original ──────────────── */}
      <div className="createNewPurchase-upload-container">
        <input
          type="file"
          ref={fileInputRef}
          ...
          onChange={handleFileChange}
          multiple
          disabled={inputDisable || uploading}
        />
        <div
          className="createNewPurchase-upload-btn"
          onClick={!inputDisable ? handleUploadClick : undefined}
          style={{ opacity: inputDisable ? 0.5 : 1, cursor: inputDisable ? "not-allowed" : "pointer" }}
        >
          {/* File-stack SVG — unchanged from original */}
          <svg
            xmlns="http://www.w3.org/2000/svg"
            width="18"
            height="20"
          ...
          >
            ...
            />
            <defs>
              <linearGradient id="paint0_linear_888_3201" x1="18.8" y1="17.5" x2="20.639" y2="3.773" gradientUnits="userSpaceOnUse">
                <stop stopColor="#BB45EA" />
                <stop offset="1" stopColor="#9C6CFE" />
              </linearGradient>
              <linearGradient id="paint1_linear_888_3201" x1="17" y1="6.5" x2="14" y2="6.5" gradientUnits="userSpaceOnUse">
                <stop offset="0.338" stopColor="#5750E2" stopOpacity="0" />
                <stop offset="1" stopColor="#5750E2" />
              </linearGradient>
              <radialGradient id="paint2_radial_888_3201" cx="0" cy="0" r="1" gradientUnits="userSpaceOnUse" gradientTransform="translate(5.05 12) rotate(-52.6548) scale(14.4655 23.3167)">
                <stop offset="0.228" stopColor="#2764E7" />
                <stop offset="0.685" stopColor="#5CD1FF" />
                <stop offset="1" stopColor="#6CE0FF" />
              </radialGradient>
              <linearGradient id="paint3_linear_888_3201" x1="3.857" y1="6" x2="3.857" y2="25.091" gradientUnits="userSpaceOnUse">
                <stop offset="0.241" stopColor="#FFD638" />
                <stop offset="0.637" stopColor="#FAB500" />
                <stop offset="0.985" stopColor="#CA6407" />
              </linearGradient>
          ...
          <nav>{uploading ? "Uploading..." : "Upload Files"}</nav>
        </div>
      </div>

      {/* ── Files pending upload (local File objects) ──────────────────────── */}
      {uploadedFiles.length > 0 && (
        <>
          <nav className="createNewPurchase-attached-files">
            {/* Attachment clip icon — unchanged from original */}
            <svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 18 18" fill="none">
              <path d="M9.79 5.09172L5.18334 9.69755C4.77579 10.1052 4.54687 10.6581 4.54695 11.2345C4.54703 11.811 4.77609 12.3638 5.18375 12.7713C5.59141 13.1789 6.14428 13.4078 6.72072 13.4077C7.29716 13.4076 7.84995 13.1785 8.2575 12.7709L13.2425 7.78589C13.5968 7.43354 13.8779 7.01464 14.0697 6.55328C14.2615 6.09193 14.3603 5.5972 14.3603 5.09755C14.3603 4.5979 14.2615 4.10318 14.0697 3.64182C13.8779 3.18047 13.5968 2.76157 13.2425 2.40922C12.5255 1.7141 11.5633 1.3299 10.5647 1.34004C9.56613 1.35018 8.61188 1.75385 7.90917 2.46339L2.88084 7.39505C1.8968 8.42033 1.35383 9.79047 1.36845 11.2115C1.38307 12.6325 1.9541 13.9912 2.95903 14.996C3.96395 16.0008 5.3227 16.5717 6.74372 16.5862C8.16475 16.6007 9.53484 16.0575 10.56 15.0734L16.7075 8.98005" stroke="black" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" />
            </svg>
            <p>Currently Attached Files:</p>
          </nav>
          {uploadedFiles.map((file, index) => (
            <div key={`upload-${index}`} className="createNewPurchase-file-item">
              <nav>{file.name}</nav>
              <div className="createNewPurchase-file-actions">
                <a
                  href={URL.createObjectURL(file)}
                ...
                <button
                  onClick={(e) => { e.preventDefault(); removeUploadedFile(index); }}
                >
                  Remove
                </button>
              </div>
      ...
      {backendFiles.length > 0 && (
        <>
          ...
          {backendFiles.map((file, index) => (
            <div key={`backend-${index}`} className="createNewPurchase-file-item">
              <nav>
                {file.name}
                {file.created_at && (
              ...
              <div className="createNewPurchase-file-actions">
                ...
                <button
                  onClick={(e) => { e.preventDefault(); removeBackendFile(index); }}
                  disabled={inputDisable}
                >
                  Remove
                </button>
      ...
      {uploadedFiles.length === 0 && backendFiles.length === 0 && (
        <p style={{ color: "#888", fontSize: "13px", marginTop: "8px" }}>
          {poId ? "No attachments yet." : "Save as Draft to enable attachments."}
        </p>
      )}
    </div>
  );
