...
export default function PurchaseHistory({ poId }) {
  const [statusChange, setStatusChange] = useState([]);
  const [email, setEmail]               = useState([]);
  const [print, setPrint]               = useState([]);
  const [loading, setLoading]           = useState(false);

  // ── Fetch history when PO id is available ────────────────────────────────
  useEffect(() => {
    if (!poId) return;
    const loadHistory = async () => {
      setLoading(true);
      const res = await purchaseOrderApiProvider.fetchHistory(poId);
      setLoading(false);

      // The API may return a flat array or a shaped object.
      // Case 1 — shaped object: { status: [...], email: [...], print: [...] }
      if (res?.status || res?.email || res?.print) {
        setStatusChange(res.status || []);
        setEmail(res.email       || []);
        setPrint(res.print       || []);
        return;
      }

      // Case 2 — flat array of history events; split by type
      const items = Array.isArray(res)
        ? res
        : res?.results || res?.history || [];
      ...
      items.forEach((item) => {
        // Build a readable label from whatever fields the API sends
        const label = item.label || item.remarks ||
          `${item.action || item.event || item.status || "Action"} by ${
            item.changed_by || item.user || "System"
          } (${
        ...
        const type = (item.action || item.event || "").toLowerCase();

        if (type.includes("email") || type.includes("mail")) {
          emailItems.push(label);
        } else if (type.includes("pdf") || type.includes("print")) {
          printItems.push(label);
        } else {
          statusItems.push(label);
        }
      });

      setStatusChange(statusItems);
      setEmail(emailItems);
      setPrint(printItems);
    };
  ...
  // ── Shared row renderer (profile icon + text) — matches original ─────────
  const renderRow = (text, ind) => (
    <nav key={ind}>
      <svg
        className="createNewPurchase-history-profile-logo"
        xmlns="http://www.w3.org/2000/svg"
  ...
  return (
    <>
      <div className="createNewPurchase-history-container">
          ...
          <div className="createNewPurchase-status-detailes">
            {!poId ? (
              <p style={{ color: "#888", fontSize: "13px" }}>
                Save as Draft to view history.
              </p>
            ) : loading ? (
              <p style={{ color: "#888", fontSize: "13px" }}>Loading...</p>
            ) : statusChange.length > 0 ? (
              statusChange.map((ele, ind) => renderRow(ele, ind))
            ) : (
              <p style={{ color: "#888", fontSize: "13px" }}>No status changes yet.</p>
            )}
          </div>
        </div>
        ...
        <div className="createNewPurchase-statusChange-container">
          <div className="createNewPurchase-status-head">
            {/* Mail icon — unchanged from original */}
            <svg
              className="createNewPurchase-history-mail-logo"
              xmlns="http://www.w3.org/2000/svg"
              width="20"
          ...
          <div className="createNewPurchase-status-detailes">
            {!poId ? (
              ...
              <p style={{ color: "#888", fontSize: "13px" }}>Loading...</p>
            ) : email.length === 0 && print.length === 0 ? (
              <p style={{ color: "#888", fontSize: "13px" }}>No email or print history yet.</p>
            ) : (
              <>
                {email.map((ele, ind) => renderRow(ele, ind))}
                {print.map((ele, ind) => renderRow(ele, ind))}
              </>
            )}
          </div>
        </div>
