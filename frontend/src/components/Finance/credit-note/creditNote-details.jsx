...
export default function CreditNoteDetails({
  ...
  // Pre-fill when editing an existing credit note
  useEffect(() => {
    if (isEdit && Object.keys(editCreditNoteData).length > 0) {
      const raw = editCreditNoteData._raw || editCreditNoteData;
      setRecordId(editCreditNoteData.id || raw.id || null);
      setcreditNoteInput((prev) => ({
        ...prev,
        creditNote_id:       editCreditNoteData.creditNote_id   || raw.CRN_ID            || "",
        invoiceReference_id: editCreditNoteData.invoice_ref     || raw.invoice_ref        || "",
        creditNote_date:     editCreditNoteData.creditNote_date || raw.credit_note_date   || "",
        custoner_name:       editCreditNoteData.customer_name   || raw.customer_name      || "",
        customer_id:         raw.customer_id                                              || "",
        due_date:            editCreditNoteData.due_date        || raw.due_date           || "",
        ...
        invoice_status:      editCreditNoteData.status          || raw.status             || "",
        payment_status:      editCreditNoteData.payment_status  || raw.payment_status     || "",
        invoice_total:       raw.invoice_total  || raw.grand_total                        || "",
      }));
      setCreditNoteStatus(editCreditNoteData.status || raw.status || "");

      // Load line items
      if (Array.isArray(raw.items)) setInvoiceListData(raw.items);
    }
  }, [isEdit]);
  ...
  useEffect(() => {
    ...
    setInvoiceBtn((prev) => statusMap[creditNoteStatus] || prev);
  }, [creditNoteStatus]);

  const handleInputChange = (e) => {
    const { id, value } = e.target;
    setcreditNoteInput((prev) => ({ ...prev, [id]: value }));
  };

  const buildPayload = () => ({
    credit_note_date: creditNoteInput.creditNote_date,
    invoice_ref:      creditNoteInput.invoiceReference_id,
    created_by:       creditNoteInput.created_by,
    branch:           creditNoteInput.branch,
    currency:         creditNoteInput.currency,
    customer_name:    creditNoteInput.custoner_name,
    customer_id:      creditNoteInput.customer_id,
    billing_address:  creditNoteInput.billing_address,
    phone_number:     creditNoteInput.phone_number,
    invoice_date:     creditNoteInput.invoice_date,
    due_date:         creditNoteInput.due_date,
    payment_terms:    creditNoteInput.payment_terms,
    payment_status:   creditNoteInput.payment_status,
    invoice_total:    creditNoteInput.invoice_total,
    refund_mode:      creditNoteInput.refund_mode,
    refund_paid:      creditNoteInput.refund_paid,
    refund_date:      creditNoteInput.refund_date,
    adjusted_invoice_ref: creditNoteInput.adjusted_invoice_ref,
  });
  ...
  const handleSaveDraftState = async (e) => {
    ...
    try {
      let result;
      if (recordId) {
        result = await creditNoteApiProvider.updateCreditNote(recordId, buildPayload());
      } else {
        result = await creditNoteApiProvider.createCreditNote(buildPayload());
        if (result?.id) setRecordId(result.id);
      }
      if (result) setCreditNoteStatus("Draft");
    } finally {
      setSaving(false);
    }
  };

  // Mark as Paid — perform action on existing record
  const handlePaidState = async (e) => {
    e.preventDefault();
    if (!recordId) {
      toast.error("Save the credit note first before marking as paid.");
      return;
    }
    setSaving(true);
    try {
      const result = await creditNoteApiProvider.performAction(recordId, "mark_paid");
      if (result) setCreditNoteStatus("Paid");
    } finally {
      setSaving(false);
    }
  };
  ...
  const handleCancelledState = async (e) => {
    e.preventDefault();
    if (!recordId) {
      toast.error("Save the credit note first before cancelling.");
      return;
    }
    const confirmed = window.confirm("Are you sure you want to cancel this credit note? This action cannot be undone.");
    if (!confirmed) return;
    setSaving(true);
    try {
      const result = await creditNoteApiProvider.performAction(recordId, "cancel");
      if (result) setCreditNoteStatus("Cancelled");
    } finally {
      setSaving(false);
    }
  };
  ...
  const handleDownloadPDF = async (e) => {
    e.preventDefault();
    if (!recordId) return;
    await creditNoteApiProvider.downloadPDF(recordId, creditNoteInput.creditNote_id);
  };

  const handleSendEmail = async (e) => {
    e.preventDefault();
    if (!recordId) return;
    await creditNoteApiProvider.sendEmail(recordId);
  };
  ...
  return (
    <div className="createNewInvoice-container">
      <form onSubmit={(e) => e.preventDefault()}>
        ...
        <div className="createNewInvoice-input-container">
          <div className="createNewInvoice-input-box">
            <label htmlFor="invoiceReference_id">Invoice Reference ID (Auto Generate)</label>
            <input
          ...
          <div className="createNewInvoice-input-box">
            ...
            <input
              ...
              value={creditNoteInput.created_by}
              onChange={handleInputChange}
              required
              disabled={InvoiceBtn.buttonAcs}
            />
        ...
        <div className="createNewInvoice-input-container">
          <div className="createNewInvoice-input-box">
            ...
            <input
              ...
              value={creditNoteInput.payment_status}
              onChange={handleInputChange}
              required
              disabled={InvoiceBtn.buttonAcs}
            />
        ...
        <div className="credit-note-container">
          <div className="credit-note-box">
            <div className="credit-note-field">
              ...
              <div className="readonly-value">{creditNoteInput.invoice_total || "—"}</div>
            </div>
            <div className="credit-note-field">
              ...
              <input
                ...
                value={creditNoteInput.refund_paid}
                onChange={handleInputChange}
                disabled={InvoiceBtn.buttonAcs}
              />
            </div>
          ...
          <div className="credit-note-box">
            <div className="credit-note-field">
              ...
              <select
                id="refund_mode"
                value={creditNoteInput.refund_mode}
                onChange={handleInputChange}
                disabled={InvoiceBtn.buttonAcs}
              >
            ...
            <div className="credit-note-field">
              ...
              <input
                id="refund_date"
                type="date"
                value={creditNoteInput.refund_date}
                onChange={handleInputChange}
                disabled={InvoiceBtn.buttonAcs}
              />
            </div>
            <div className="credit-note-field credit-note-adjusted-reference">
              ...
              <textarea
                ...
                value={creditNoteInput.adjusted_invoice_ref}
                onChange={handleInputChange}
                disabled={InvoiceBtn.buttonAcs}
              />
            </div>
        ...
        <div className="createNewInvoice-hub-container">
          ...
          <div className="createNewInvoice-hub-body">
            {detail.comment && <InvoiceComment creditNoteId={recordId} apiProvider={creditNoteApiProvider} />}
            {detail.history && <InvoiceHistory creditNoteId={recordId} apiProvider={creditNoteApiProvider} />}
            {detail.attachment && <InvoiceAttachment creditNoteId={recordId} apiProvider={creditNoteApiProvider} />}
          </div>
        </div>
