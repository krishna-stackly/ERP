...
// import React, { useState, useEffect } from "react";
// import "./deliveryNoteCRM.css";
// import useDeliveryNoteStore from "./deliveryNoteStore";
...
//   const [deleteDNData, setDeleteDNData] = useState(null);

//   // =====================================================
//   // FETCH ON MOUNT
//   // =====================================================
//   useEffect(() => {
//     fetchDeliveryNotes(1, "");
//   }, []);
...
//   // =====================================================
//   useEffect(() => {
//     const selectedOrders = deliveryNotes.filter(
//       (order) => buttonAct.checkbox[order.dn_id]
//     );
//     const hasMultiple = selectedOrders.length > 1;
//     const firstDN_ID = selectedOrders[0]?.sales_order_ref;
...
//       invoice: selectedOrders.length > 0 && validStatus && sameDN_ID,
//     }));
//   }, [buttonAct.checkbox, deliveryNotes]);
...
//   // =====================================================
//   const handleNext = () => {
//     if (currentPage < totalPages) fetchDeliveryNotes(currentPage + 1, search);
//   };

//   const handlePrev = () => {
//     if (currentPage > 1) fetchDeliveryNotes(currentPage - 1, search);
//   };
...
//     if (!deleteDNData) return;
//     await deleteDeliveryNote(deleteDNData.dn_id);
//     fetchDeliveryNotes(currentPage, search);
//     setShowDeleteModal(false);
//     setDeleteDNData(null);
//   };

//   // =====================================================
//   // CLIENT-SIDE FILTERING
//   // =====================================================
//   const filteredData = deliveryNotes.filter((item) => {
...
//               }}
//             >
//               Are you sure you want to delete delivery note <br />
//               <strong>{deleteDNData?.dn_id}</strong>
//               <br />
//               (Customer: {deleteDNData?.customer_name})?
//             </p>
...
//         <div className="deliveryCRM-header">
//           <p>Delivery Notes</p>
//           <button onClick={() => setCurrentPage("createNewDelivery")}>
//             + New Delivery Note
//           </button>
//         </div>
...
export default function DeliveryNoteCRM({ setCurrentPage }) {
  ...
  useEffect(() => {
    ...
    const hasMultiple = selectedOrders.length > 1;
    const firstRef = selectedOrders[0]?.sales_order_ref;
    const sameDN_ID =
      hasMultiple &&
      selectedOrders.every((o) => o.sales_order_ref === firstRef);
    const validStatus =
      hasMultiple &&
      selectedOrders.every(
        (o) =>
  ...
  return (
    <>
        ...
        <>
          ...
          >
            ...
            <div style={{ display: "flex", justifyContent: "center", gap: "12px" }}>
              <button
                onClick={() => setShowDeleteModal(false)}
                style={{ padding: "8px 24px", borderRadius: "6px", border: "1px solid #ccc", background: "#f5f5f5", cursor: "pointer", fontWeight: 500 }}
              >
                Cancel
              </button>
              <button
                onClick={confirmDelete}
                style={{ padding: "8px 24px", borderRadius: "6px", border: "none", background: "#e53935", color: "#fff", cursor: "pointer", fontWeight: 500 }}
              >
                Delete
              </button>
            </div>
      ...
      <div className="deliveryCRM-container">
        ...
        <div className="deliveryCRM-header">
          <p>Delivery Notes</p>
          <button onClick={() => setCurrentPage("deliveryForm", null)}>
            + New Delivery Note
          </button>
        </div>
        ...
        <div className="deliveryCRM-search-category">
          <div className="deliveryCRM-input-box">
            ...
            <select
              value={filter.delivery_status}
              onChange={(e) => setFilter((p) => ({ ...p, delivery_status: e.target.value }))}
              id="delivery_status"
            >
              <option value="">All</option>
              <option value="Draft">Draft</option>
          ...
          <div className="deliveryCRM-input-box">
            ...
            <select
              value={filter.delivery_type}
              onChange={(e) => setFilter((p) => ({ ...p, delivery_type: e.target.value }))}
              id="delivery_type"
            >
              <option value="">All Types</option>
              <option value="Regular">Regular</option>
          ...
          <div className="deliveryCRM-input-box">
            ...
            <nav id="delivery_date">
              <div>
                ...
                <input
                  value={filter.delivery_from_date}
                  onChange={(e) => setFilter((p) => ({ ...p, delivery_from_date: e.target.value }))}
                  className="deliveryCRM-date"
                  type="date"
                />
              </div>
              <div>
                ...
                <input
                  value={filter.delivery_to_date}
                  onChange={(e) => setFilter((p) => ({ ...p, delivery_to_date: e.target.value }))}
                  className="deliveryCRM-date"
                  type="date"
                />
              </div>
        ...
        <div className="deliveryCRM-table-title">
          <nav>
            <button
              className={buttonAct.delivery_return ? "deliveryCRM-active-btn" : "deliveryCRM-inactive-btn"}
              disabled={!buttonAct.delivery_return}
            >
              Generate Delivery Return
            </button>
            <button
              className={buttonAct.invoice ? "deliveryCRM-active-btn" : "deliveryCRM-inactive-btn"}
              disabled={!buttonAct.invoice}
            >
              Generate Invoice
            </button>
                  ...
                  <tr key={ele.dn_id || ind}>
                    ...
                    <td>
                      <p
                        className={`deliveryCRM-Status ${
                          ...
                          : ele.status === "Delivered" ? "deliveryCRM-Status-Delivered"
                          : ele.status === "Cancelled" ? "deliveryCRM-Status-Cancelled"
                          : ele.status === "Partially Delivered" ? "deliveryCRM-Status-partiallyDelivered"
                          : ele.status === "Returned" ? "deliveryCRM-Status-Returned"
                          : ""
                        }`}
                      >
                        {ele.status}
                    ...
                    <td style={{ position: "relative", textAlign: "center" }}>
                      <div
                        onClick={(e) => {
                          e.stopPropagation();
                          setOpenMenuId(openMenuId === ele.dn_id ? null : ele.dn_id);
                        }}
                        style={{
                          display: "inline-flex",
                          alignItems: "center",
                          ...
                          borderRadius: "50%",
                          cursor: "pointer",
                          background: openMenuId === ele.dn_id ? "#f0f0f0" : "transparent",
                        }}
                      >
                      ...
                      {openMenuId === ele.dn_id && (
                        ...
                        >
                          ...
                          <div
                            onClick={() => {
                              setOpenMenuId(null);
                              setCurrentPage("deliveryForm", ele);  // ✅ pass full row object
                            }}
                          ...
                          <div
                            onClick={() => {
                              if (["Partially Delivered", "Delivered", "Returned"].includes(ele.status)) {
                                setOpenMenuId(null);
                                // handle generate delivery return
                              }
                            }}
                            style={{
                              padding: "10px 16px",
                              cursor: ["Partially Delivered", "Delivered", "Returned"].includes(ele.status) ? "pointer" : "not-allowed",
                              fontSize: "14px",
                              color: ["Partially Delivered", "Delivered", "Returned"].includes(ele.status) ? "#333" : "#bbb",
                            }}
                            onMouseEnter={(e) => {
                              if (["Partially Delivered", "Delivered", "Returned"].includes(ele.status))
        ...
        <nav className="deliveryCRM-table-bottem">
          ...
          <div className="deliveryCRM-manage-control-box">
            <button className="deliveryCRM-manage-btn" onClick={handlePrev} disabled={currentPage <= 1}>
              Prev
            </button>
            <nav className="deliveryCRM-num-page">
              Page {currentPage} of {totalPages}
            </nav>
            <button className="deliveryCRM-manage-btn" onClick={handleNext} disabled={currentPage >= totalPages}>
              Next
            </button>
          </div>
        </nav>
