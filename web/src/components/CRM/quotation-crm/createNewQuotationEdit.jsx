...
// Usage — Create: <CreateNewQuotation setshowNewQuotation={...} />
// Usage — Edit:   <CreateNewQuotation setshowNewQuotation={...} editQuotationData={row} isEdit />
export default function CreateNewQuotation({
  ...
  const [createdId, setCreatedId]   = useState(isEditMode ? editQuotationData.id || null : null);
  ...
  useEffect(() => {
    async function loadDropdowns() {
      try {
        ...
        // Fallback while endpoint is being built
  ...
  useEffect(() => {
    if (isEditMode) {
      ...
      setCreatedId(editQuotationData.id        || null);
    }
  ...
  useEffect(() => {
    const defaultState = {
      saveDraft: true, submit: true, approve: true, reject: true,
      salesOrder: true, pdf: false, email: false, historyBtn: true, reviseBtn: false,
    };
  ...
  useEffect(() => {
    setinputDisable(
      ["Send", "Approved", "Rejected", "Converted (SO)", "Expired"].includes(status)
    );
  }, [status]);
  ...
  // =========================================================================
  // SAVE / UPDATE — shared by all action buttons
  // =========================================================================
  async function saveQuotation(newStatus) {
    ...
    try {
      ...
      let result;
      if (createdId) {
        result = await quotationApiProvider.updateQuotation(createdId, payload);
      } else {
        result = await quotationApiProvider.createQuotation(payload);
        if (result) {
          const newId = result?.data?.id || result?.id;
          if (newId) setCreatedId(newId);
          const qId = result?.data?.quotation_id || result?.quotation_id || "";
          if (qId) setNewQuotationData((prev) => ({ ...prev, quotation_id: qId }));
        }
      }
  ...
  // ── Status badge class ────────────────────────────────────────────────────
  const statusClass = {
    Draft:           "newQuotation-status-bg-draft",
    Send:            "newQuotation-status-bg-send",
    Approved:        "newQuotation-status-bg-approved",
    Rejected:        "newQuotation-status-bg-rejected",
    "Converted (SO)":"newQuotation-status-bg-converted",
    Expired:         "newQuotation-status-bg-expired",
  }[status] || "";

  const isModalOpen = showRevise || showHistory;
  ...
  return (
    <>
      ...
      <div className={`newQuotation-container ${isModalOpen ? "newQuotation-blur" : ""}`}>
        <form onSubmit={handleSubmitState}>
          ...
          <div className="newQuotation-hub-container">
            <div className="newQuotation-hub-head">
              {[
                ...
                <p
                  key={label}
                  className={active ? "newQuotation-hub-head-bg-black" : "newQuotation-hub-head-tit"}
                  onClick={onClick}
                >
                  {label}
                </p>
            ...
            <div className="newQuotation-hub-body">
              {comment    && <CreateNewQuotationComments    quotationId={createdId} />}
              {history    && <CreateNewQuotationHistory     quotationId={createdId} />}
              {attachment && <CreateNewQuotationAttachment  quotationId={createdId} inputDisable={inputDisable} />}
            </div>
          </div>
          ...
          <div className="newQuotation-btn-container">
            ...
            {/* PDF icon */}
            <svg
              className={`newQuotation-pdf-mail-activelogo ${buttonState.pdf ? "newQuotation-pdf-mail-futurelogo" : ""}`}
              style={{ cursor: !buttonState.pdf ? "pointer" : "default" }}
              onClick={() => !buttonState.pdf && quotationApiProvider.generatePdf(createdId)}
              xmlns="http://www.w3.org/2000/svg"
              viewBox="0 0 22 24"
              fill="none"
            >
            ...
            {/* Mail icon */}
            <svg
              className={`newQuotation-pdf-mail-activelogo ${buttonState.email ? "newQuotation-pdf-mail-futurelogo" : ""}`}
              style={{ cursor: !buttonState.email ? "pointer" : "default" }}
              onClick={() => !buttonState.email && quotationApiProvider.sendEmail(createdId)}
              xmlns="http://www.w3.org/2000/svg"
              viewBox="0 0 20 16"
              fill="none"
            >
